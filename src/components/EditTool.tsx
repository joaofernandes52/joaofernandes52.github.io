...
function ApprovalFooter({
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-2.5 py-2 border-t border-neutral-200 bg-neutral-50 ">
      ...
      {decision === null && (
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="px-2 h-7 rounded-[6px] text-xs font-medium border border-neutral-200 text-neutral-700 hover:bg-neutral-100"
          >
            {rejectLabel}
          </button>
          <button
            ...
            className="px-2 h-7 rounded-[6px] text-xs font-medium bg-blue-500 text-white hover:bg-blue-600 "
          >
            {approveLabel}
          </button>
        </div>
...
export const EditTool = React.memo(function EditTool({
  ...
  return (
    <div
      className={cn(
        "rounded-[10px] border border-neutral-200 bg-white overflow-hidden w-full shadow-xl shadow-neutral-200/50",
        className,
      )}
    >
      <div
        className={cn(
          "flex items-center justify-between px-3 h-8 bg-neutral-100",
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          {isPending || isWaiting ? (
            <span className="an-edit-shimmer text-[13px] font-medium">{headerLabel}</span>
          ) : (
            <span className="text-[13px] font-medium text-neutral-600 truncate">
              {headerLabel}
            </span>
          )}
        </div>
      ...
      {diffOps && diffOps.length > 0 && (
        <div className="text-[12px] font-mono leading-[1.6] bg-white overflow-x-auto p-2">
          {diffOps.map((op, i) => (
            <div
              key={i}
              className={cn(
                "flex items-start min-w-0 px-2 py-0.5 rounded-sm",
                op.type === "add" &&
            ...
            >
              <span
                className={cn(
                  "select-none w-4 text-center shrink-0 opacity-70",
                  op.type === "add" && "text-emerald-600",
                  op.type === "remove" && "text-rose-600",
                  op.type === "context" && "text-neutral-400",
                )}
              >
